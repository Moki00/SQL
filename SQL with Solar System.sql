...
INSERT INTO solar_system_objects
    (body,mean_radius,mean_radius_rel,volume,volume_rel,mass,mass_rel,density,surface_gravity,surface_gravity_rel,type_of_object,shape)
VALUES
...
SELECT body, ROUND(MAX(mass))
FROM solar_system_objects;
SELECT body, MIN(mass)
FROM solar_system_objects;

SELECT ROUND(SUM(mass)) AS Total_Mass_of_Solar_System
FROM solar_system_objects;

SELECT body, SUM(mass) AS total_mass
FROM solar_system_objects
GROUP BY body
HAVING total_mass>500 AND total_mass<123456;

SELECT body, surface_gravity,
    CASE 
        WHEN surface_gravity > 20 THEN "Too Strong"
        WHEN surface_gravity > 7 THEN "Comfortable"
        WHEN surface_gravity > 1 THEN "Super Jump"
        ELSE "Dont Jump"
    END as "Pull/Push"
FROM solar_system_objects;
...
SELECT body, number_grade,
    CASE 
        WHEN fraction_completed THEN ROUND(fraction_completed*100)
...
CASE 
        WHEN ROUND
(fraction_completed*100)>70 THEN 'C'
        WHEN ROUND
(fraction_completed*100)>80 THEN 'B'
        WHEN ROUND
(fraction_completed*100)>90 THEN 'A'
        ELSE 'F'
END as "percent_completed"
FROM solar_system_objects;
...
SELECT body, surface_gravity,
    CASE 
        WHEN surface_gravity > 220-30 THEN "Too Strong"
        WHEN surface_gravity > ROUND(0.90 * (220-30)) THEN "Comfortable"
        WHEN surface_gravity > ROUND(0.50 * (220-30)) THEN "Dont Jump"
        ELSE "Float By"
    END as "Gravity"
FROM solar_system_objects;
